...
public class Main5{
	...
	private static int obstaclesNumber;
	...
	private static void parseArgs(String[] args) {
		...
		cmdLineOptions.addOption(constructMultiViewOption()); // -m or --multiviews
	...
	private static Option constructObstaclesOption() {
		 String optionInfo = "Activate obstacles for ATAXX game";
	     Option opt = new Option("o", "obstacles", true, optionInfo);
	     opt.setArgName("Obstacles");
	     return opt;
	}
	
	private static void parseObstaclesOption(CommandLine line) throws ParseException {
        String obstacles = line.getOptionValue("o");
        if (obstacles != null) {
        	try {
                obstaclesNumber = Integer.parseInt(obstacles);
               
            } catch (NumberFormatException e) {
                throw new ParseException("Enter a integer for obtacles option");
            }
        }
    }
	...
	private static void parseGameOption(CommandLine line) throws ParseException {
		...
		case ATAXX:
			if (dimRows != null && dimCols != null && dimRows == dimCols) {
				gameFactory = new AtaxxFactoryExt(dimRows, obstaclesNumber);
	...
	private static void parseHelpOption(CommandLine line, Options cmdLineOptions) {
		if (line.hasOption("h")) {
			...
			formatter.printHelp(Main5.class.getCanonicalName(), cmdLineOptions, true);
			System.exit(0);
		}
	}
		...
		switch (view) {
			...
			case WINDOW:
				...
				for (int i = 0; i < pieces.size(); i++){
					switch (playerModes.get(i)){
					case AI:
						player.add(gameFactory.createAIPlayer(aiPlayerAlg));
						break;
					case MANUAL:
						player.add(gameFactory.createConsolePlayer());
						break;
					case RANDOM:
						player.add(gameFactory.createRandomPlayer());
						break;
					default:
						throw new UnsupportedOperationException("Something went wrong! This program point should be unreachable!");
				}
